...
class OnlineTrainer(Trainer):
        ...
        for i in range(self.cfg.eval_episodes):
            ...
            while not done:
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to convert obs to tensor in eval(): {e}"
                    ) from e
    ...
    def to_td(self, obs, action=None, reward=None, terminated=None):
        ...
        if isinstance(obs_val, np.ndarray):
            ...
            raise TypeError(
                "obs must be numpy array, torch tensor, or dict containing them"
            )
        ...
        while self._step <= self.cfg.steps:
            ...
            if self._step > self.cfg.seed_steps:
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to convert obs to tensor for agent.act(): {e}"
                    ) from e
